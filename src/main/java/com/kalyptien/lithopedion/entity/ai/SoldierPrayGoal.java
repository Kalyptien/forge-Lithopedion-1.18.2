...
public class SoldierPrayGoal  extends MoveToBlockGoal {
    private SoldierEntity soldier = null;
    private int prayTimer = 0;
    private int cooldown = 0;

    public SoldierPrayGoal(SoldierEntity pMob, double speed, int length) {
    ...
    public void stop() {
        ...
        if(!this.soldier.isConnect()){
            ...
            if(block instanceof SanctuaryAutelBlock){
                ...
                this.soldier.setSanctuaryVariant(((SanctuaryAutelBlock) block).getSvariant());
            }
        }
        else{
            Level level = this.soldier.level;
            BlockState blockstate = level.getBlockState(this.blockPos);
            Block block = blockstate.getBlock();
            if(block instanceof SanctuaryStatusBlock){
                ((SanctuaryStatusBlock) block).setJadeProcess(blockstate, 2);
            }
        }
    ...
    protected boolean isValidTarget(LevelReader pLevel, BlockPos pPos) {
        if(soldier.isConnect()){
            return (pLevel.isEmptyBlock(pPos.above()) && pLevel.isEmptyBlock(pPos.above().above()) && pLevel.getBlockState(pPos).getBlock() instanceof SanctuaryStatusBlock && pLevel.getBlockState(pPos).getValue(SanctuaryStatusBlock.JADE_PROCESS) == 1 && ((SanctuaryStatusBlock) pLevel.getBlockState(pPos).getBlock()).getSanctuary() == soldier.getSanctuary()) ||
                    (pLevel.isEmptyBlock(pPos.above()) && pLevel.isEmptyBlock(pPos.above().above()) && pLevel.getBlockState(pPos).getBlock() == soldier.getSanctuary());
        }
        else{
